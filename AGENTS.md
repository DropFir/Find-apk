...
6. 根据一体化工具结果立即分流：
   ...
   - 下载工具报告 `missing its required ABI split` 时，说明当前 `.apk` 只是 App Bundle 的 base APK，不是成功交付。若同一精确页存在同版本 XAPK/APKM/APKS，必须立刻以正确扩展名重新调用 `download_from_page.py`；在完整拆分包保存并验证前禁止换关键词、写人工说明或保留该 base APK 作为推荐文件。
...
6. 多关键词搜索与候选验证总时限不得超过 `关键词数量 × 150 秒`；已验证文件的 60–900 秒实际传输预算单独计算。每完成一个关键词都要检查该关键词及整批耗时，不能把保存图标和最终汇总放到无上限的尾部阶段。
...
12. 多关键词任务的完成标准是用户列表中的每个关键词都已保存并验证安装包，或付费应用已按规则跳过。`download-note.txt` 只表示仍在处理，不能使关键词达到终态。只完成前几项时只能发送进度说明，不得输出整批最终答复、进入空闲状态或把剩余项目留给用户再次提醒。
...
确认开发者和目标应用相符，保存 WEBP 图标和 `developer.txt`，并在对话中给出官网、Google Play 与下载来源。只有保存并验证一个安装包，或按官方付费规则跳过，才算完成。自动下载暂时失败时可保存 `download-note.txt` 记录人工入口和阻塞原因，但任务保持未完成并继续处理。
