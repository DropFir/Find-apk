...
9. 运行到 120 秒仍没有已验证的可下载文件直链时，必须停止搜索。已经取得正确直链并开始的大文件传输按独立预算继续；没有直链时剩余 30 秒只用于保存图标、`developer.txt`、`download-note.txt` 和回复。
10. 到 150 秒必须结束当前关键词的搜索与候选验证；Cloudflare 后备的合计 75 秒上限包含在内，不因浏览器、镜像报错或图标转换继续延长。已验证并开始的 50 MiB 以上文件传输是唯一例外，最多延长到工具计算的 900 秒上限。
11. Pillow 等固定依赖应在开始整批关键词前检查并安装一次，不得在单个关键词计时过程中重复准备环境。
12. 多关键词任务的完成标准是用户列表中的每个关键词都已保存安装包，或在自动流程穷尽后保存对应 `download-note.txt`。只完成前几项时只能发送进度说明，不得输出整批最终答复、进入空闲状态或把剩余项目留给用户再次提醒。
