...
传入 APKPure 精确详情页也可以：如果详情页只包含“Download APK/XAPK”中间入口，工具会自动追加 `/download`、重新解析公开 `d.apkpure.*` 文件链接并继续下载。输出会包含 `transition=apkpure_download_page`。只有实际 `/download` 页仍然返回 `browser_required` 时才允许使用 Chrome，并且 Chrome 必须打开 `/download` 页，不能重开详情页。
...
`/download` 页并提取 `d.apkpure.*` 公开文件链接；该确定性转换不再依赖 Agent 或 Chrome。
...
传输预算提升到 60 秒，一体化工具的父进程也会等待完整预算。最终仍因可重试网络
...
- 普通公开文件直链每次最多 20 秒；50 MiB 及以上安装包最多 60 秒，单次命令内只重试一次。
