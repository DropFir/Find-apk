...
class CodexController:
    ...
    def _worker_prompt(self, worker_id: str) -> str:
        return (
            ...
            "来源顺序必须采用 build_source_searches.py 的当前输出：先执行 Google Web Alternatives、"
            "APKMirror、AndroidAPKs、CNET、Softonic 等替代路径；标记为 cloudflare-last 的"
            "APKCombo、APKPure、APKPure.net、Uptodown 放到最后。不得因为记得或已经知道某个"
            "APKPure 候选页就跳过排在前面的 Google 与较低阻塞来源。"
